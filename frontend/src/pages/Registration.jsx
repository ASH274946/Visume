...
const Step2 = ({ onNext, onBack, role, formData, updateFormData }) => {
  ...
  const handleDocumentUpload = async (e, type) => {
    ...
    try {
      ...
      if (type === 'aadhaar') {
        // Regex for 12 digits, allowing spaces or hyphens between every 4 digits
        const aadhaarRegex = /(?:\d{4}[\s-]?){2}\d{4}|\b\d{12}\b/g;
        const matches = text.match(aadhaarRegex);
        if (matches && matches.length > 0) {
           extractedValue = matches[0].replace(/[\s-]/g, '');
           // Format beautifully
           extractedValue = `${extractedValue.slice(0, 4)} ${extractedValue.slice(4, 8)} ${extractedValue.slice(8, 12)}`;
        }
      } else if (type === 'pan') {
