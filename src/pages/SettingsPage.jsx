...
const CandidateSidebar = () => (
  <aside className="w-[240px] flex-shrink-0 bg-surface-container border border-outline-variant flex flex-col h-[calc(100vh-32px)] my-4 ml-4 rounded-2xl shadow-xl shadow-black/40 z-50">
    ...
    <nav className="flex-1 mt-sm flex flex-col gap-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
      <Link className="flex items-center gap-3 text-text-muted px-4 py-3 hover:bg-surface-bright/5 hover:text-text-primary transition-all duration-300 translate-x-0 hover:translate-x-1" to="/dashboard">
    ...
    <div className="flex flex-col gap-1 mb-2">
      <Link className="flex items-center gap-3 bg-sidebar-active-glow text-primary-container border-l-4 border-primary-container px-4 py-3 transition-all duration-300" to="/settings" state={{ role: 'candidate' }}>
...
const RecruiterSidebar = () => (
  <aside className="w-[240px] flex-shrink-0 bg-surface-container border border-outline-variant flex flex-col h-[calc(100vh-32px)] my-4 ml-4 rounded-2xl shadow-xl shadow-black/40 z-50">
    ...
    <div className="flex flex-col gap-1 mb-2">
      ...
      <Link className="flex items-center gap-3 bg-sidebar-active-glow text-primary-container border-l-4 border-primary-container px-4 py-3 font-label-md text-label-md transition-all duration-300" to="/settings" state={{ role: 'recruiter' }}>
        <span className="material-symbols-outlined">settings</span>
        Settings
      </Link>
    </div>
...
const SettingsPage = () => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background text-text-primary font-body-md selection:bg-primary/30 selection:text-primary antialiased">
      {role === 'recruiter' ? <RecruiterSidebar /> : <CandidateSidebar />}
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar p-lg">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-md mb-lg border-b border-outline-variant pb-md">
          ...
          <div className="flex items-center bg-surface-container-low p-1 rounded-xl border border-border-input">
            <button
              type="button"
              onClick={() => handleRoleToggle('candidate')}
              className={`px-4 py-2 rounded-lg font-label-md text-label-md font-bold transition-all flex items-center gap-2 ${role === 'candidate' ? 'bg-primary-container text-white shadow-lg' : 'text-text-muted hover:text-text-primary'}`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-label-md text-label-md font-bold transition-all flex items-center gap-2 ${role === 'recruiter' ? 'bg-primary-container text-white shadow-lg' : 'text-text-muted hover:text-text-primary'}`}
            >
